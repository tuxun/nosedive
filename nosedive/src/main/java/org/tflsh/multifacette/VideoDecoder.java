...
public class VideoDecoder {
  ...
  private static final MediaCodec.CryptoInfo sCryptoInfo = new MediaCodec.CryptoInfo();
  ...
  private VideoDecoder(MediaCodec codec) {
    mDecoder = codec;
    codec.start();
  ...
  /**
   * Constructs the {@link VideoDecoder} wrapper object around the video codec.
   ...
   */
  public static VideoDecoder fromVideoFormat(final MediaFormat trackFormat,
      Surface surface) throws IOException {
    VideoDecoder result = null;
    ...
    // newly created codec.
    if (videoCodec != null) {
      result = new VideoDecoder(videoCodec);
    }
  ...
  public void stopAndRelease() {
    //    mDecoder is already null at end of video
    if (mDecoder != null) {
      mDecoder.stop();
      mDecoder.release();
    }
  ...
  public boolean writeSample(final ByteBuffer input,
    ...
    if (size > 0 && !mAvailableInputBuffers.isEmpty()) {
      ...
      if (size > buffer.capacity()) {
        throw new VideoDecoder.WriteException(String.format(Locale.US,
   ...
   * OutputSampleListener#outputSample(VideoDecoder, android.media.MediaCodec.BufferInfo,
   * java.nio.ByteBuffer)}
   *
            ...
            mHandler.post(new Runnable() {
              ...
              public void run() {
                mOutputFormatChangedListener
                    .outputFormatChanged(VideoDecoder.this,
  ...
  public interface OutputFormatChangedListener {
    void outputFormatChanged(VideoDecoder sender, MediaFormat newFormat);
  }
  ...
  public interface OutputSampleListener {
    void outputSample(VideoDecoder sender, MediaCodec.BufferInfo info, ByteBuffer buffer);
  }
